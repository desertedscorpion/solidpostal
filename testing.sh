...
docker build -t ninthgrimmercury/solidpostal . &&
    ...
    sleep 30s &&
    if [[ "HTTP/1.1 200 OK" == $(curl --head http://127.0.0.1:28860 | head --lines 1) ]]
    then
    ...
    if [[ "HTTP/1.1 200 OK" == $(curl --head http://127.0.0.1:28860/credential-store/domain/_/credential/79ad7607-ef6e-4e5f-a139-e633aded192b/ | head --lines 1) ]]
    then
    ...
    if [[ "HTTP/1.1 200 OK" == $(curl --head http://127.0.0.1:28860/computer/slave/ | head --lines 1) ]]
    then
    ...
    if [[ "HTTP/1.1 200 OK" == $(curl --header http://127.0.0.1:28860/job/job/ | head --lines 1) ]]
    then
    ...
    if [[ "HTTP/1.1 200 OK" == $(curl --head http://127.0.0.1:28860/job/git1/ws/Dockerfile/*view*/ | head --lines 1) ]]
    then
    ...
    if [[ "HTTP/1.1 200 OK" == $(curl --head http://127.0.0.1:28860/job/git2/ws/Dockerfile/*view*/) ]]
    then
